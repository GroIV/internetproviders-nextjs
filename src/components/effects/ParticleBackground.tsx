...
export function ParticleBackground({
  ...
  // Store animate function in ref to avoid recreation and allow self-reference
  useEffect(() => {
    animateFnRef.current = (ctx: CanvasRenderingContext2D, width: number, height: number) => {
  ...
  useEffect(() => {
    ...
    const rect = canvas.getBoundingClientRect()
    animateFnRef.current?.(ctx, rect.width, rect.height)
